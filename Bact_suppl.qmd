...
# model_df_complete <- 
#   model_df_complete %>%
#   mutate(t_WBC_noNEU = log(1+WBC_noNEU))

model_df <- 
  ADLB |> filter(KEY_PRED_FL03 == "Y") |>
  select(USUBJID, PARAMCD, AVAL)

model_df_complete <- model_df |> drop_na()

model_df_trans <- 
  ADLB |> filter(KEY_PRED_FL04 == "Y") |>
  select(USUBJID, PARAMCD, AVAL)

model_df_trans_complete <- model_df_trans |> drop_na()

n_excl_cases <- dim(model_df)[1] - dim(model_df_complete)[1]
 
pct_excl <-
  round((1 - (dim(model_df_complete)[1] / dim(model_df)[1])) * 100, 1)

pct_complete <- dim(model_df_complete)[1] / dim(model_df)[1] * 100

model_df |> group_by(PARAMCD) |> tally() |> gt() |> gt_theme_538()
model_df_complete |> group_by(PARAMCD) |> tally() |> gt() |> gt_theme_538()
model_df_trans |> group_by(PARAMCD) |> tally() |> gt() |> gt_theme_538()
model_df_trans_complete |> group_by(PARAMCD) |> tally() |> gt() |> gt_theme_538()

model_data <- model_df_complete |> left_join(ADSL |> select(USUBJID, BACTEREMIAN), by = "USUBJID")
model_data_transformed <- model_df_trans_complete |> left_join(ADSL |> select(USUBJID, BACTEREMIAN), by = "USUBJID")
...
model_dat_wide <- model_data |>
  pivot_wider(names_from = "PARAMCD", values_from = "AVAL")

key_predictors <- model_dat_wide |> select(-USUBJID, -BACTEREMIAN) |> names()
key_predictors
...
global_formula <- paste0('BACTEREMIAN ~ ', paste(paste0(paste0('fp(', key_predictors_trans), ',df=2)'), collapse = ' + '))
